...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    // Get province by ID
    try {
      const province = await db.select().from(provinsi).where(eq(provinsi.id, id)).limit(1)
  ...
  if (method === 'PUT') {
    ...
    try {
      ...
      const updatedProvince = await db.update(provinsi)
        .set({
          nama,
          svg_path,
          id_provinsi
  ...
  if (method === 'DELETE') {
    ...
    try {
      const deletedProvince = await db.delete(provinsi)
        .where(eq(provinsi.id, id))
        .returning()
