<template>
  <div>
    <div class="d-flex align-center mb-6">
      <v-btn icon size="small" @click="$router.push('/admin/hub-inovasi')" class="mr-3">
      ...
      <v-card-text>
        ...
        >
          ...
          <template v-slot:item.verifiedTotalScore="{ item }">
            <v-chip 
              :color="item.totalScore !== item.verifiedTotalScore ? 'orange' : 'green'" 
              size="small"
            >
              {{ item.verifiedTotalScore }}
              <v-icon v-if="item.totalScore !== item.verifiedTotalScore" size="small" class="ml-1">
                mdi-alert-circle
              </v-icon>
            </v-chip>
          </template>
      ...
      <v-card class="pa-6">
        ...
        <v-card-text class="pa-0" style="max-height: 70vh;">
          <div v-if="selectedResponse">
                  ...
                  <div>
                    ...
                    <div class="d-flex align-center gap-2">
                      ...
                      <div class="text-center">
                        ...
                        <v-chip 
                          size="small" 
                          :color="selectedResponse.totalScore !== selectedResponse.verifiedTotalScore ? 'orange' : 'green'"
                        >
                          {{ selectedResponse.verifiedTotalScore }}
                        </v-chip>
                      </div>
                      <v-chip 
            ...
            <!-- Answers by Aspect -->
            <div
              v-for="(aspect, aIndex) in selectedResponse.aspectScores"
              :key="aIndex"
              class="mb-6"
            >
              <v-card elevation="2">
                        ...
                        >
                          <div class="d-flex align-center justify-space-between">
                            <span class="text-caption">
                              Skor diubah: 
                              <span class="text-decoration-line-through">{{ getOriginalScore(question) }}</span>
                              →
                              <span class="font-weight-bold">{{ getVerifiedScore(question) }}</span>
                            </span>
                            <v-chip 
...
// State
const surveyTitle = ref('Survey Pemetaan Tingkat Kematangan HUB JIPPNAS')
const search = ref('')
...
const headers = [
  ...
  { title: 'Total Skor Responden', key: 'totalScore', width: '150px' },
...
const fetchResponses = async () => {
  ...
  try {
    ...
    if (response.success) {
      responses.value = response.data.map(item => ({
        ...item,
        verifiedTotalScore: item.verifiedTotalScore || item.totalScore
      }))
    } else {
      responses.value = []
    }
...
const saveVerification = () => {
  ...
  const allVerified = selectedResponse.value.aspectScores.every(aspect =>
    aspect.questions.every(q => q.verificationStatus === 'verified')
  )
  const anyRejected = selectedResponse.value.aspectScores.some(aspect =>
...
const calculateTotalScore = (response) => {
  if (!response || !response.aspectScores) return 0
  return response.aspectScores.reduce((total, aspect) => {
    return total + calculateAspectScore(aspect)
  }, 0)
}

const hasAnyScoreChanged = (response) => {
  if (!response || !response.aspectScores) return false
  return response.aspectScores.some(aspect => hasAspectScoreChanged(aspect))
}
</script>
